import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { motion, AnimatePresence } from 'framer-motion';
import { FaWineGlassAlt, FaArrowLeft, FaDice, FaUserPlus, FaRandom } from 'react-icons/fa';

const MostLikelyToGame = () => {
  // ================== STATE MANAGEMENT ==================
  const [players, setPlayers] = useState([]);
  const [newPlayerName, setNewPlayerName] = useState('');
  const [showPlayerSetup, setShowPlayerSetup] = useState(true);
  
  // Game round states
  const [voterOrder, setVoterOrder] = useState([]); // randomized order of voters
  const [currentVoterIndex, setCurrentVoterIndex] = useState(0);
  const [votes, setVotes] = useState({}); // vote counts keyed by player name
  const [prompt, setPrompt] = useState('');
  const [promptCount, setPromptCount] = useState(0);
  const [roundResult, setRoundResult] = useState(null); // holds result text for the round

  const navigate = useNavigate();

  // Collection of "Most Likely To..." prompts
  const prompts = [
    "Most likely to text their ex",
    "Most likely to sleep through an earthquake",
    "Most likely to win a dance-off",
    "Most likely to become a millionaire overnight",
    "Most likely to forget their own birthday",
    "Most likely to break into song randomly",
    "Most likely to get lost in a mall",
    "Most likely to binge-watch a whole series in one night"
  ];

  // ================== HELPER FUNCTIONS ==================
  // Randomly picks a prompt from the list
  const getRandomPrompt = () => {
    return prompts[Math.floor(Math.random() * prompts.length)];
  };

  // Fisher-Yates shuffle for arrays
  const shuffleArray = (array) => {
    const shuffled = [...array];
    for (let i = shuffled.length - 1; i > 0; i--) {
      const j = Math.floor(Math.random() * (i + 1));
      [shuffled[i], shuffled[j]] = [shuffled[j], shuffled[i]];
    }
    return shuffled;
  };

  // ================== PLAYER SETUP HANDLERS ==================
  const handleAddPlayer = () => {
    if (newPlayerName.trim() !== '') {
      setPlayers([...players, newPlayerName.trim()]);
      setNewPlayerName('');
    }
  };

  const handleKeyPress = (e) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      handleAddPlayer();
    }
  };

  const handleRemovePlayer = (index) => {
    setPlayers(players.filter((_, i) => i !== index));
  };

  const handleShufflePlayers = () => {
    setPlayers(shuffleArray(players));
  };

  // Start game only if at least 2 players have been added
  const handleStartGame = () => {
    if (players.length < 2) {
      window.alert("Please add at least 2 players to start the game");
      return;
    }
    setShowPlayerSetup(false);
    startNewRound();
  };

  // ================== GAME ROUND HANDLERS ==================
  // Set up a new round: choose a new prompt, reset votes and voter order, clear previous result
  const startNewRound = () => {
    setPrompt(getRandomPrompt());
    setPromptCount(prev => prev + 1);
    const initialVotes = {};
    players.forEach(player => { initialVotes[player] = 0; });
    setVotes(initialVotes);
    setVoterOrder(shuffleArray(players));
    setCurrentVoterIndex(0);
    setRoundResult(null);
  };

  // Record a vote for the selected candidate from the current voter
  const handleVote = (candidate) => {
    // Only allow voting if roundResult hasn't been set (i.e. voting is open)
    if (roundResult) return;

    const updatedVotes = { ...votes };
    updatedVotes[candidate] = (updatedVotes[candidate] || 0) + 1;
    setVotes(updatedVotes);

    const nextIndex = currentVoterIndex + 1;
    if (nextIndex >= voterOrder.length) {
      // All players have voted – calculate the result
      calculateResult(updatedVotes);
    }
    setCurrentVoterIndex(nextIndex);
  };

  // Tally votes and update the round result in the UI
  const calculateResult = (votesObj) => {
    const voteEntries = Object.entries(votesObj);
    const maxVotes = Math.max(...voteEntries.map(([_, count]) => count));
    const winners = voteEntries
      .filter(([_, count]) => count === maxVotes)
      .map(([name]) => name);
    if (winners.length === 1) {
      setRoundResult(`${winners[0]} gets the most votes! They drink!`);
    } else {
      setRoundResult(`It's a tie between ${winners.join(', ')}! Everyone drinks!`);
    }
  };

  // Handler for the Next Round button
  const handleNextRound = () => {
    startNewRound();
  };

  // Go back to the player setup screen
  const handleBack = () => {
    setShowPlayerSetup(true);
  };

  // ================== RENDERING ==================
  return (
    <div className="fixed inset-0 overflow-hidden bg-gradient-to-br from-emerald-900 via-teal-800 to-cyan-900">
      {/* Animated background elements */}
      <div className="absolute inset-0 overflow-hidden">
        {[...Array(30)].map((_, i) => (
          <div 
            key={i}
            className="absolute rounded-full opacity-10"
            style={{
              width: `${20 + Math.random() * 120}px`,
              height: `${20 + Math.random() * 120}px`,
              background: `rgba(${Math.random() * 155 + 100}, 255, ${Math.random() * 200 + 55}, 0.3)`,
              top: `${Math.random() * 100}%`,
              left: `${Math.random() * 100}%`,
              animationDuration: `${15 + Math.random() * 45}s`,
              animation: `float infinite ease-in-out`,
              animationDelay: `${Math.random() * -20}s`,
              boxShadow: '0 0 40px rgba(255, 255, 255, 0.3)',
              transform: `rotate(${Math.random() * 360}deg) scale(${0.8 + Math.random() * 0.5})`,
            }}
          />
        ))}
      </div>

      {showPlayerSetup ? (
        // ================= PLAYER SETUP SCREEN =================
        <div className="relative z-10 flex flex-col items-center justify-center min-h-[70vh] px-6">
          <motion.div 
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.5 }}
            className="bg-gradient-to-br from-emerald-700/80 to-teal-900/80 backdrop-blur-xl p-8 rounded-3xl shadow-lg max-w-md w-full mb-10 border border-emerald-500/20"
          >
            <h2 className="text-2xl text-white font-bold mb-6 text-center">Add Player Names</h2>
            
            <div className="flex mb-6">
              <input 
                type="text" 
                value={newPlayerName}
                onChange={(e) => setNewPlayerName(e.target.value)}
                onKeyPress={handleKeyPress}
                placeholder="Enter player name"
                className="flex-1 px-4 py-3 rounded-l-xl bg-emerald-800/60 text-white placeholder-emerald-300/60 border border-emerald-600/40 focus:outline-none focus:ring-2 focus:ring-emerald-400/60 transition-all duration-300"
              />
              <button 
                onClick={handleAddPlayer}
                className="bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white px-5 py-3 rounded-r-xl transition duration-300 shadow-lg"
              >
                <FaUserPlus />
              </button>
            </div>
            
            {players.length > 0 && (
              <>
                <div className="mb-4 flex justify-between items-center">
                  <h3 className="text-lg text-emerald-300 font-medium flex items-center">
                    <span className="bg-emerald-600/50 h-7 w-7 rounded-full inline-flex items-center justify-center mr-2 text-emerald-100 text-sm">{players.length}</span>
                    <span>Players</span>
                  </h3>
                  <button 
                    onClick={handleShufflePlayers}
                    className="bg-gradient-to-r from-cyan-600 to-cyan-700 hover:from-cyan-700 hover:to-cyan-800 text-white p-2.5 rounded-lg transition duration-300 flex items-center space-x-1.5 shadow-lg"
                  >
                    <FaRandom size={14} />
                    <span className="text-sm">Shuffle</span>
                  </button>
                </div>
                
                <div className="max-h-52 overflow-y-auto mb-5 bg-emerald-800/40 rounded-xl p-3 border border-emerald-600/20 shadow-inner">
                  {players.map((player, index) => (
                    <motion.div 
                      key={index} 
                      initial={{ opacity: 0, x: -10 }}
                      animate={{ opacity: 1, x: 0 }}
                      transition={{ delay: index * 0.05 }}
                      className="flex justify-between items-center p-3 mb-2 bg-emerald-700/50 rounded-lg border-l-4 border-emerald-500 hover:bg-emerald-700/70 transition duration-200"
                    >
                      <span className="text-white font-medium">{index + 1}. {player}</span>
                      <motion.button 
                        onClick={() => handleRemovePlayer(index)}
                        className="text-red-400 hover:text-red-300 hover:bg-red-500/20 h-7 w-7 rounded-full flex items-center justify-center transition-all"
                        whileHover={{ rotate: 90 }}
                        whileTap={{ scale: 0.9 }}
                      >
                        &times;
                      </motion.button>
                    </motion.div>
                  ))}
                </div>
              </>
            )}
            
            <motion.button 
              onClick={handleStartGame}
              disabled={players.length < 2}
              className={`w-full py-3.5 rounded-xl text-white font-medium transition duration-300 mt-4 shadow-lg ${
                players.length >= 2 
                  ? 'bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700' 
                  : 'bg-gray-600/70 cursor-not-allowed'
              }`}
            >
              Start Game
            </motion.button>
          </motion.div>
          
          <motion.button
            onClick={() => navigate('/')}
            whileHover={{ scale: 1.05 }}
            whileTap={{ scale: 0.95 }}
            className="bg-gradient-to-r from-red-500 to-red-600 text-white px-6 py-3.5 rounded-xl shadow-lg hover:from-red-600 hover:to-red-700 transition duration-300 flex items-center"
          >
            <FaArrowLeft className="mr-2" />
            Back
          </motion.button>
        </div>
      ) : (
        // ================= GAME SCREEN =================
        <div className="relative z-10 flex flex-col items-center justify-between min-h-[70vh] px-6 pb-8">
          {/* Top stats section */}
          <div className="w-full">
            <div className="flex justify-center space-x-5 mb-6 mt-2">
              <motion.div 
                initial={{ opacity: 0, y: -10 }}
                animate={{ opacity: 1, y: 0 }}
                className="bg-emerald-800/60 px-5 py-2 rounded-full text-emerald-200 text-sm shadow-lg border border-emerald-600/30 flex items-center"
              >
                <FaDice className="mr-2 text-emerald-400" />
                <span>Prompts: {promptCount}</span>
              </motion.div>
              <motion.div 
                initial={{ opacity: 0, y: -10 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.1 }}
                className="bg-emerald-800/60 px-5 py-2 rounded-full text-emerald-200 text-sm shadow-lg border border-emerald-600/30 flex items-center"
              >
                <FaUserPlus className="mr-2 text-emerald-400" />
                <span>Players: {players.length}</span>
              </motion.div>
            </div>
          </div>
          
          {/* Middle section: prompt card, current voter info, voting buttons, and result display */}
          <div className="flex-1 flex items-center justify-center w-full">
            <AnimatePresence mode="wait">
              <motion.div 
                key={prompt + currentVoterIndex}
                initial={{ y: 20, opacity: 0, scale: 0.9 }}
                animate={{ y: 0, opacity: 1, scale: 1 }}
                exit={{ y: -20, opacity: 0, scale: 0.9 }}
                transition={{ duration: 0.4 }}
                className="bg-gradient-to-br from-emerald-700/80 to-teal-900/80 backdrop-blur-xl p-8 rounded-3xl shadow-lg max-w-md w-full border border-emerald-500/20"
              >
                {/* Current voter badge or "All votes in" indicator */}
                {currentVoterIndex < voterOrder.length ? (
                  <motion.div 
                    className="bg-gradient-to-r from-emerald-600 to-teal-600 px-6 py-2.5 rounded-full text-center mb-7 mx-auto inline-block shadow-lg border border-emerald-500/30"
                    initial={{ opacity: 0, y: -15 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ delay: 0.1 }}
                  >
                    <span className="text-white font-medium">{voterOrder[currentVoterIndex]}</span>
                  </motion.div>
                ) : (
                  <motion.div 
                    className="bg-gradient-to-r from-emerald-600 to-teal-600 px-6 py-2.5 rounded-full text-center mb-7 mx-auto inline-block shadow-lg border border-emerald-500/30"
                    initial={{ opacity: 0, y: -15 }}
                    animate={{ opacity: 1, y: 0 }}
                    transition={{ delay: 0.1 }}
                  >
                    <span className="text-white font-medium">All votes in!</span>
                  </motion.div>
                )}
                
                <div className="flex justify-center mb-6">
                  <motion.div
                    initial={{ rotate: -10, scale: 0.9 }}
                    animate={{ rotate: 0, scale: 1 }}
                    transition={{ delay: 0.2 }}
                    className="bg-emerald-600/40 p-4 rounded-full"
                  >
                    <FaWineGlassAlt className="text-emerald-300 text-2xl drop-shadow-lg" />
                  </motion.div>
                </div>
                
                <motion.p 
                  className="text-l text-center mb-6 text-white font-medium leading-relaxed"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  transition={{ delay: 0.2 }}
                >
                  {prompt}
                </motion.p>
                
                <motion.div 
                  className="text-center text-emerald-200 mb-2 border-t border-emerald-600/50 pt-5 mt-6"
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  transition={{ delay: 0.3 }}
                >
                  <p className="text-xs italic bg-emerald-800/40 inline-block px-4 py-2 rounded-lg">
                    Everyone, vote for who is most likely!
                  </p>
                </motion.div>
                
                {/* Voting buttons appear only if voting is still open */}
                {currentVoterIndex < voterOrder.length && !roundResult && (
                  <div className="grid grid-cols-2 gap-4">
                    {players.map((player, index) => (
                      <button
                        key={index}
                        onClick={() => handleVote(player)}
                        className="bg-blue-500 text-white p-2 rounded hover:bg-blue-600 transition duration-200"
                      >
                        {player}
                      </button>
                    ))}
                  </div>
                )}
                
                {/* Show round result if available */}
                {roundResult && (
                  <div className="mt-4 p-3 bg-blue-200 text-blue-800 rounded text-center font-semibold">
                    {roundResult}
                  </div>
                )}
              </motion.div>
            </AnimatePresence>
          </div>
          
          {/* Bottom section: navigation buttons */}
          <div className="text-xs w-full flex justify-center mt-8">
            <div className="flex items-center space-x-6">
              <motion.button
                onClick={handleBack}
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="bg-gradient-to-r from-red-500 to-red-600 text-white px-6 py-3.5 rounded-xl shadow-lg hover:from-red-600 hover:to-red-700 transition duration-300 flex items-center"
              >
                <FaArrowLeft className="mr-2" />
                Back
              </motion.button>
              
              {/* Only show "Next Round" button once the result is displayed */}
              {roundResult && (
                <motion.button
                  onClick={handleNextRound}
                  whileHover={{ scale: 1.05 }}
                  whileTap={{ scale: 0.95 }}
                  className="bg-gradient-to-r from-emerald-500 to-teal-600 text-white px-8 py-4 rounded-xl shadow-lg hover:from-emerald-600 hover:to-teal-700 transition duration-300 flex items-center font-medium"
                >
                  <FaDice className="mr-2 text-lg" />
                  Next Round
                </motion.button>
              )}
            </div>
          </div>
        </div>
      )}
      
      {/* CSS for animated background */}
      <style jsx>{`
        @keyframes float {
          0%, 100% {
            transform: translateY(0) rotate(0deg);
          }
          33% {
            transform: translateY(-15px) rotate(3deg);
          }
          66% {
            transform: translateY(-25px) rotate(-3deg);
          }
        }
      `}</style>
    </div>
  );
};

export default MostLikelyToGame;